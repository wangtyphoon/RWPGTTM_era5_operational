...
def process_data_step(
    ...
    current_features = next_data[prev_step_idx]['grid'].x[:, idx["time_start"]:idx["tisr_idx"]]
    ...
    tisr_old = current_data['grid'].x[:, idx["tisr_idx"]]
    ...
    current_data['grid'].x = torch.cat([first116, current_features,tisr_next.unsqueeze(1), prev_features, tisr_old.unsqueeze(1), graph_feature, step_feat], dim=1)
    return current_data
