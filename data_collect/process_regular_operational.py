...
def process_regular(start_date, end_date, input_dir=None, output_base=None):
    ...
    if output_base is None:
        output_base = os.path.join(
            current_dir, "..", "npz", "operational", "sfc", "regular"
        )
    os.makedirs(output_base, exist_ok=True)

    for year, month in iter_months(start_date, end_date):
        file_path = os.path.join(input_dir, f"{year:04d}-{month:02d}_regular.grib")
        if not os.path.exists(file_path):
            print(f"{file_path} not found, skipping.")
            continue

        print(f"Processing file: {file_path}")
        ds = xr.open_dataset(file_path, engine="cfgrib").drop_vars(
            ["meanSea", "swh"], errors="ignore"
        )

        ds_swh = xr.open_dataset(
            file_path,
            engine="cfgrib",
        ...
        for valid_time in ds.coords["time"].values:
            ...
            subset = ds.sel(time=valid_time)
            subset_swh = ds_swh.sel(time=valid_time)
            ...
            arr_swh = subset_swh["swh"].values.astype(np.float32)
            var_dict["swh"] = np.squeeze(arr_swh)
...
if __name__ == "__main__":
    process_regular("2024-01-01", "2024-01-01")
