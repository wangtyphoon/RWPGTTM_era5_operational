...
class GATv2ConvN(MessagePassing):
    ...
    def forward(  # noqa: F811
        ...
        out = torch.cat([out, x_r], dim=-1) if self.residual else out
...
if __name__ == '__main__':
    ...
    # Random inputs
    x = torch.randn(num_nodes, in_feat, device=device)
    # Random directed edges: shape (2, num_edges)
    num_edges = 100000
    ...
    edge_attr = torch.randn(edge_index.size(1), edge_feat_dim, device=device)

    # Instantiate and move conv to GPU
    conv = GATv2ConvN(
        in_channels=in_feat,
        out_channels=256,
        heads=1,
    ...
    # Warm‐up (optional, clears any lazy allocations)
    _ = conv(x, edge_index, edge_attr)

    # Reset stats and run forward
    torch.cuda.reset_peak_memory_stats(device)
